...
pub struct MemoryPlatform {
    ...
    /// 机制式排队合并开关（v0.4.7）：true=批=连续处理组（飞行缓冲消除队列空隙）；
    /// false=完全回退逐条现状。
    merge_enabled: bool,
}
...
impl MemoryPlatform {
    ...
    pub fn spawn(mut self) -> tokio::task::JoinHandle<()> {
        tokio::spawn(async move {
            tracing::info!(
                "memory_platform: started, polling rx (merge_enabled={})",
                self.merge_enabled
            );
            let heartbeat =
            ...
            if self.merge_enabled {
                ...
                loop {
                    ...
                    for (i, msg) in batch.into_iter().enumerate() {
                        ...
                        // 待处理消息数口径 = 飞行缓冲 + 本批剩余（长跑积压随处理连续推进回稳）。
                        queue.absorb_channel(&mut self.memory_rx);
...
mod tests {
    ...
    fn memory_turn_context(id: &str) -> crate::agent::communication::TurnContext {
    ...
    #[tokio::test]
    async fn merged_loop_processes_each_turn_in_order_and_finishes() {
        // v0.4.7 机制式合并（记忆中台）：批 = 连续处理组，批内逐轮沉淀 + 逐轮 memory_complete。
        // 无洞察输出 → fallback 路径（fallback_memory + finish_turn），每轮独立收口。
        let (pool, _receivers) = AgentPool::new();
        let pool = Arc::new(pool);
        for t in ["t1", "t2", "t3", "t4"] {
            pool.create_turn_context(memory_turn_context(t)).await;
        }
        let (tx, rx) = mpsc::channel(8);
        let platform = memory_platform_with(rx, pool.clone());
        let handle = platform.spawn();

        for t in ["t1", "t2", "t3"] {
            tx.send(AgentMessage::InsightDone { turn_id: t.into() })
                .await
                .unwrap();
        }
        // 处理中补发 t4（飞行缓冲路径）。
        tx.send(AgentMessage::InsightDone {
            turn_id: "t4".into(),
        })
        .await
        .unwrap();
        ...
        for t in ["t1", "t2", "t3", "t4"] {
            ...
            assert_eq!(ctx.status, crate::agent::communication::TurnStatus::Done);
        }
    }
    ...
    async fn merged_loop_single_turn_degradation_identical() {
        ...
        let (pool, _receivers) = AgentPool::new();
        let pool = Arc::new(pool);
        pool.create_turn_context(memory_turn_context("t1")).await;
        let (tx, rx) = mpsc::channel(8);
        let platform = memory_platform_with(rx, pool.clone());
        let handle = platform.spawn();
        ...
        assert!(ctx.memory.is_some(), "单条退化行为与逐条现状一致");
        assert_eq!(ctx.status, crate::agent::communication::TurnStatus::Done);
    }

    #[tokio::test]
    async fn merge_disabled_falls_back_to_per_item_processing() {
        // merge_enabled=false：完全回退逐条现状（v0.4.6 及以前行为）。
        let (pool, _receivers) = AgentPool::new();
        let pool = Arc::new(pool);
        for t in ["t1", "t2", "t3"] {
            pool.create_turn_context(memory_turn_context(t)).await;
        ...
        for t in ["t1", "t2", "t3"] {
            let ctx = pool.get_turn_context(t).await.unwrap();
            assert!(ctx.memory.is_some(), "回退模式逐条处理: {t}");
            assert_eq!(ctx.status, crate::agent::communication::TurnStatus::Done);
        }
    }
}
